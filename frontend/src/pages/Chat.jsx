...
export const Chat = () => {
  ...
  function handleSelected(id, options) {
    setSelected(id)
    setOptions(options)
  }
  ...
  return (
    <div className="chatpage" style={{ width: '100%', backgroundColor: 'white', marginTop: '22px', border: '1px solid #dbdbdb', display: 'flex', flexDirection: 'row', height: '90vh', marginBottom: '3vh', borderRadius: '4px' }}>
      <div className="left_chat_bar" style={{ width: '33%', borderRight: '1px solid #dbdbdb', height: '100%', overflowY: 'scroll' }}>
        ...
        {
          rooms?.map(item => <RoomName handleSelected={handleSelected} key={item.roomId} roomId={item.roomId} people={item.people} />)
        }
      </div>
      <div className="right_chatbar" style={{ width: '67%' }}>
        <ChatBox roomId={selected} options={options} />
      </div>
    </div>
  )
}
